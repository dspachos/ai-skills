...
try {
  if (args.includes('all')) {
    ...
    console.log('\nClearing CSS and JavaScript aggregates...');
    try {
      execSync('drush css:flush', { encoding: 'utf8' });
      execSync('drush js:flush', { encoding: 'utf8' });
      console.log('Asset clearing complete');
    } catch (err) {
      console.log('Asset clearing commands not available');
    }
  } else {
